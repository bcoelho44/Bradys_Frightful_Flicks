class Movie < ApplicationRecord
  ...
  validates :stock_quantity, numericality: { only_integer, greater_than_or_equal_to: 0 }
  ...
  def self.ransackable_scopes(auth_object = nil)
    %i[subgenre_id_eq recently_added]
  end
  ...
  # Additional scope for filtering by keyword in title or description
  scope :keyword_search, ->(keyword) {
    where("title ILIKE :keyword OR description ILIKE :keyword", keyword: "%#{keyword}%")
  }
end
